...
<Quote variant="warning">
  Для продакшн использование самоподписанных сертификатов не рекомендуется. Подобные сертификаты не
  обеспечивают надежной защиты и не признаются учреждениями сертификации.
</Quote>
...
<CodeBlock path="./docker-compose.yml">
```yaml
...
<CodeBlock path="./frontend/Dockerfile">
```dockerfile
...
<CodeBlock path="./proxy/Dockerfile">
```dockerfile
...
<CodeBlock path="./proxy/nginx/default.conf.tpl">
```bash
...
<CodeBlock path="./proxy/nginx/proxy_params">
```bash
